#!/usr/bin/env bash
# Enhanced Kernel Build Script (v5 - Separate git/build cleaning)
# - KernelSU (main) integration
# - SuSFS patches for android14-6.1
# - Ask for: 1) Git source clean  2) Build directory clean
...
: "${PIXEL8A:=y}"
: "${LTO_TYPE:=thin}"
: "${ZIP_PREFIX:=AK3-A14-6.1.155-KSU}"
...
prepare_kernel_source() {
  ...
  log_step "Git source preparation"
  echo -e "${YELLOW}Choose source preparation:${ENDCOLOR}" >&2
  echo "  1) Clean (git clean + git reset + remove KernelSU) - Fresh code" >&2
  echo "  2) Skip clean (keep modifications + keep KernelSU)" >&2
  ...
  case "$choice" in
    1)
      log_info "Starting source clean"
    ...
    2)
      ...
      # Check if SUSFS patches are already applied
      if grep -q "CONFIG_KSU_SUSFS" "$CONFIG_FILE" 2>/dev/null; then
        log_warn "SUSFS patches appear to be already applied!"
        log_warn "Skipping patch reapplication to prevent duplicates"
        SKIP_SUSFS_PATCHES=1
      fi
      rm -rf KernelSU || true
...
install_kernelsu() {
  ...
  retry "$RETRY" bash -c \
    "curl -LSs https://raw.githubusercontent.com/tiann/KernelSU/refs/heads/${KSUN_BRANCH}/kernel/setup.sh | bash -s ${KSUN_BRANCH}"
...
# ---------- SUSFS patches ----------
apply_susfs_patches() {
  if [[ "${SKIP_SUSFS_PATCHES:-0}" == "1" ]]; then
    log_info "Skipping SUSFS patches (already applied)"
    return
  fi
  
  log_step "Applying SUSFS patches"
  cp -fv ../susfs4ksu/kernel_patches/KernelSU/10_enable_susfs_for_ksu.patch ./
  cp -fv ../0001-KSU-kernel-side.patch ./
  ...
  log_info "Applying 0001-KSU-kernel-side.patch"
  patch -p1 -f < 0001-KSU-kernel-side.patch || log_warn "Patch 0001-KSU-kernel-side encountered issues, continuing"
...
configure_kernel() {
  ...
  # === SUSFS - Default configuration ===
  append_unique_cfg "$CONFIG_FILE" "CONFIG_KSU_SUSFS=y"
  append_unique_cfg "$CONFIG_FILE" "CONFIG_KSU_SUSFS_HAS_MAGIC_MOUNT=n"
...
prepare_build() {
  ...
  # Ask about build directory cleanup
  if [[ -d "$OUT_DIR" ]]; then
    log_step "Build directory cleanup"
    echo -e "${YELLOW}OUT_DIR exists: $OUT_DIR${ENDCOLOR}" >&2
...
# ---------- Package ----------
get_kernelsu_version_from_git() {
  # Try multiple possible KernelSU locations
  local possible_dirs=(
    "$KERNEL_DIR/KernelSU"      # Inside kernel directory
    "KernelSU"                    # In parent directory
    "./KernelSU"                  # Current directory
  )
  
  for ksu_dir in "${possible_dirs[@]}"; do
    if [[ -d "$ksu_dir/.git" ]]; then
      log_info "Found KernelSU git at: $ksu_dir" >&2
      local commit_count=$(cd "$ksu_dir" 2>/dev/null && git rev-list --count HEAD 2>/dev/null || true)
      
      if [[ -n "$commit_count" && "$commit_count" =~ ^[0-9]+$ ]]; then
        local version=$((20000 + commit_count))
        log_info "Calculated version: r$version (20000 + $commit_count commits)" >&2
        printf "r%s" "$version"
        return 0
      fi
    fi
  ...
  log_warn "KernelSU git directory not found in any location" >&2
  return 1
}
...
package_kernel() {
  ...
  log_info "Looking for Image at: $image"
  ls -lh "$image" || { log_error "Kernel image not found: $image"; exit 1; }
  
  log_info "Copying Image to $ANYKERNEL_DIR"
  cp -fv "$image" "$ANYKERNEL_DIR"/ || { log_error "Failed to copy Image"; exit 1; }
  
  (cd "$ANYKERNEL_DIR"; rm -rf .git; )

  # Try to detect version automatically
  local release_num=""
  
  log_step "Detecting KernelSU version..."
  
  if release_num=$(get_kernelsu_version_from_build "$KERNEL_DIR/$OUT_DIR" 2>/dev/null); then
    log_info "✓ Extracted from build log: $release_num"
  elif release_num=$(get_kernelsu_version_from_git 2>/dev/null); then
    log_info "✓ Calculated from git commits: $release_num"
  else
    log_error "Could not detect KernelSU version"
    exit 1
  fi
  
  [[ -n "$release_num" ]] || { log_error "release_num is empty"; exit 1; }
  log_info "Final release number: $release_num"
  
  # Include hours and minutes in the build label
  local date_str="$(ts_date)"
  local time_str="$(TZ=UTC date -u +"%H%M")"
  local BUILD_LABEL="${date_str}-${time_str}-${release_num}"
  local zip_name="${ZIP_PREFIX}-${BUILD_LABEL}.zip"
  
  log_info "Creating zip: $zip_name"
  log_info "AnyKernel dir contents:"
  ls -la "$ANYKERNEL_DIR" | head -10
  
  (cd "$ANYKERNEL_DIR" && zip -r "../$zip_name" ./* 2>&1) || { log_error "zip command failed"; exit 1; }

  mkdir -p "$BUILDS_DIR" || { log_error "Cannot create builds dir"; exit 1; }
  mv -v "$zip_name" "$BUILDS_DIR"/ || { log_error "Cannot move zip"; exit 1; }
